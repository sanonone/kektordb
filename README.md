...
*   **One-liner setup for AI coding agents:**
    ...
    ```
    Writes MCP config with `--tools=agent` (17 tools) to keep agent context lean. Idempotent -- safe to re-run.
...
*   [x] **MCP One-Liner:** `kektordb setup <agent>` configures MCP for Claude Code, Cursor, Gemini CLI, Codex, and OpenCode in a single command. `--tools` flag with `agent`/`admin`/`all` tool profiles (17/6/23 tools).
