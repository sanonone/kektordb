...
from kektordb_client import APIError, KektorDBClient
...
class DBRunner(abc.ABC):
    ...
    @abc.abstractmethod
    def index_vectors(
        self, index_name: str, vectors: np.ndarray, metric: str, config: dict
    ):
        pass

    @abc.abstractmethod
    def search(
        self, index_name: str, query_vectors: np.ndarray, k: int, config: dict
    ) -> (float, list):
        pass
...
class KektorDBRunner(DBRunner):
    def __init__(self, host="localhost", port=9091):
        self.client = KektorDBClient(host, port, timeout=None)
    ...
    def index_vectors(self, index_name, vectors, metric, config):
        ...
        self.client.recreate_collection(
            ...
            hnsw_config=qdrant_models.HnswConfigDiff(
                m=config.get("m", 16), ef_construct=config.get("ef_construction", 200)
            ),
    ...
    def search(self, index_name, query_vectors, k, config):
        ...
        for q in query_vectors[:10]:
            self.client.query_points(
                index_name, query=q.tolist(), limit=k, search_params=search_params
            )
...
class ChromaDBRunner(DBRunner):
    ...
    def index_vectors(self, index_name, vectors, metric, config):
        ...
        dist = (
            "l2" if metric == "euclidean" else metric
        )  # Chroma usa 'cosine' o 'l2' o 'ip'
...
if __name__ == "__main__":
    ...
    if QDRANT_AVAILABLE:
        runners.append(QdrantRunner())
    if CHROMA_AVAILABLE:
        runners.append(ChromaDBRunner())
