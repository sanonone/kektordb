...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100000,
        help="Dataset size (keep small for quick refine test)",
    )
    ...
    client.vcreate(
        ...
        precision="float32",
        m=16,
        ef_construction=BAD_EF_CONSTRUCTION,
    )

    batch_data = [
        {"id": words[i], "vector": vectors[i].tolist()} for i in range(len(vectors))
    ]
    BATCH_SIZE = 10000

    for i in range(0, len(batch_data), BATCH_SIZE):
        client.vadd_batch(index_name, batch_data[i : i + BATCH_SIZE])
    ...
    # 3. Recall Base
    base_recall = calc_recall(
        client, index_name, queries, gt_sets, "BASELINE (Low Quality)"
    )

    # 4. Configurazione Refine
    print(f"\nConfigurazione Refine (ef={TARGET_EF}, batch={REFINE_BATCH})...")
    client.vupdate_config(
        index_name,
        {
            "refine_enabled": True,
    ...
    # 6. Recall Finale
    final_recall = calc_recall(
        client, index_name, queries, gt_sets, "REFINED (High Quality)"
    )
    ...
    if improvement > 0.01:
        ...
        print(
            "⚠️ ATTENZIONE: Miglioramento trascurabile (il grafo era già buono o refine non efficace)."
        )
