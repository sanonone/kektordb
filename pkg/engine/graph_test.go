...
func TestBidirectionalGraph(t *testing.T) {
	...
	if err := eng.VLink(idx, source, target, rel, "", 1.0, nil); err != nil {
		t.Fatalf("VLink failed: %v", err)
	}

	// 3. Test Forward Link (Did we save 'rel'?)
	// Should return ["entity_Go"]
	links, found := eng.VGetLinks(idx, source, rel)
	...
	incoming, found := eng.VGetIncoming(idx, target, rel)
	...
	// Verify cleanup
	_, foundFwd := eng.VGetLinks(idx, source, rel)
	if foundFwd {
		t.Error("Forward link should have been deleted")
	}
	_, foundRev := eng.VGetIncoming(idx, target, rel)
	if foundRev {
