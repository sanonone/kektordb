...
// cleanLLMJSONResponse extracts the first JSON object from an LLM response,
// stripping markdown code fences (```json ... ```), leading/trailing
// prose, and trailing commas. This is essential when using small local
// models (e.g. gemma4, llama3) that often wrap JSON in markdown or add
// explanatory text around it.
//
// Returns the cleaned JSON string, or an empty string if no JSON object
...
Respond with JSON:
{
  ...
  "profile_data": "FULL JSON STRING FOR LLM"
	...
	// Clean LLM response: strip markdown code fences and extract JSON.
	// Small local models (gemma4, llama3) often wrap JSON in ```json ... ```
	// which causes json.Unmarshal to fail with "invalid character '`'".
	cleaned := cleanLLMJSONResponse(respText)
	if cleaned == "" {
		slog.Warn("[Cognitive Engine] No JSON object found in LLM profile response",
