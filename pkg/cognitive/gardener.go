...
// cleanLLMJSONResponse extracts the LAST valid JSON object from an LLM response,
// stripping <think>...</think> reasoning blocks, markdown code fences
// (```json ... ```), and surrounding prose. This is essential when using
// reasoning models (deepseek-v4, deepseek-r1) that output chain-of-thought
// before the final answer, and small local models (gemma4, llama3) that
// often wrap JSON in markdown or add explanatory text around it.
//
// The parser uses string-aware brace matching to find balanced JSON objects,
// returning the LAST one that successfully unmarshals. Reasoning models put
// the answer at the end; the prose before may contain `{}` or other
// non-answer JSON that should be skipped.
//
// Returns the cleaned JSON string, or an empty string if no valid JSON
// object is found.
func cleanLLMJSONResponse(respText string) string {
	trimmed := strings.TrimSpace(respText)

	// 1. Strip <think>...</think> blocks (deepseek-r1/v4 reasoning)
	for {
		openIdx := strings.Index(trimmed, "<think>")
		if openIdx == -1 {
			break
		}
		closeIdx := strings.Index(trimmed[openIdx:], "</think>")
		if closeIdx == -1 {
			// Unclosed think block — drop everything from <think> onwards
			trimmed = strings.TrimSpace(trimmed[:openIdx])
			break
		}
		// Remove the entire think block
		trimmed = trimmed[:openIdx] + trimmed[openIdx+closeIdx+len("</think>"):]
	...
	if strings.HasPrefix(trimmed, "```") {
		...
		if strings.HasPrefix(strings.TrimSpace(trimmed), "json") {
			if idx := strings.Index(trimmed, "\n"); idx != -1 {
				trimmed = trimmed[idx+1:]
			}
		}
	...
	// 3. Brace-match: find all complete JSON objects, return the LAST valid one.
	return extractLastJSONObject(trimmed)
}

// extractLastJSONObject scans text for balanced `{...}` JSON objects
// (string-aware: ignores braces inside "...") and returns the last one
// that successfully unmarshals. Returns "" if none found.
...
type Gardener struct {
	...
	// Profile update debouncer. Instead of spawning one goroutine per threshold-cross
	// (which causes N concurrent LLM calls when N saves arrive in quick succession),
	// we wait for a quiet period before calling UpdateUserProfile. This avoids:
	...
	// Users with different IDs remain parallel.
	profileUpdateLocks sync.Map // userID -> *sync.Mutex
}
...
func (g *Gardener) trackUserInteraction(event engine.Event) {
	...
	// Check if memory has user_id. In MCP mode, any memory with user_id
	// is implicitly a user interaction — the old "user_interaction" tag
	// requirement was an HTTP-mode concept that silently dropped all
	// MCP memories. Now any save_memory with user_id counts.
	...
	slog.Debug("[Cognitive Engine] User interaction tracked",
		"user", userID,
		"unassimilated", count,
		"threshold", g.cfg.ProfileUpdateThreshold,
	)

	// If threshold reached, debounce the profile update. Instead of spawning
	// one goroutine per threshold-cross (which causes N concurrent LLM calls
	// when N saves arrive quickly), we wait for a quiet period and then
	// do ONE update with all accumulated interactions. This avoids:
	//   - Rate-limit issues with cloud LLM providers
	//   - Wasted tokens (each call would include the same prior interactions)
	//   - Race conditions on the profile node (concurrent writes)
	if count >= g.cfg.ProfileUpdateThreshold {
		g.scheduleProfileUpdate(event.IndexName, userID, count)
	}
}

// scheduleProfileUpdate debounces profile updates: it starts (or resets)
// a timer, and only when the timer fires does it actually call
// UpdateUserProfile. Rapid bursts of save_memory calls result in a
// single update with all accumulated interactions, not N parallel calls.
func (g *Gardener) scheduleProfileUpdate(indexName, userID string, count int) {
	g.profileTimerMu.Lock()
	defer g.profileTimerMu.Unlock()

	// Track this (indexName, userID) pair as pending. If multiple users
	// trigger profile updates close together, the timer still fires once
	// but each unique user is processed.
	...
	slog.Info("[Cognitive Engine] Profile update scheduled (debounced)",
		"user", userID,
		"interactions", count,
		"debounce", g.profileDebounce,
	)
}

// flushPendingProfileUpdates is the callback that fires after the
// debounce quiet period. It drains the pending users map, then calls
// UpdateUserProfile for each (one goroutine per user, but they run
...
func (g *Gardener) flushPendingProfileUpdates(count int) {
	...
	if count > 0 {
		slog.Info("[Cognitive Engine] Profile update debounce fired",
			"users", len(pending),
			"last_count", count,
		)
	} else {
		slog.Info("[Cognitive Engine] Flushing pending profile updates on shutdown",
			"users", len(pending),
		)
	}

	for userID, indexName := range pending {
		go g.UpdateUserProfile(indexName, userID)
	}
}
...
func (g *Gardener) think() {
	...
	if err != nil || len(indexInfos) == 0 {
		slog.Debug("[Cognitive Engine] No vector indexes found in DB")
		return
	}

	// Diagnostic: log which indexes exist and which we'll process
	for _, info := range indexInfos {
		slog.Debug("[Cognitive Engine] Index candidate",
			"name", info.Name,
			"vectors", info.VectorCount,
			"allowed", g.cfg.isIndexAllowed(info.Name),
		)
	}
	...
	for _, info := range indexInfos {
		...
		if !g.cfg.isIndexAllowed(idx) {
			slog.Debug("[Cognitive Engine] Skipping index (not in target list)", "index", idx, "targets", g.cfg.TargetIndexes)
			continue
		}

		// 2. Skip indexes with too few vectors to produce meaningful clusters.
		if info.VectorCount < 3 {
			slog.Info("[Cognitive Engine] Skipping index (below min vector threshold)",
				"index", idx,
				"vectors", info.VectorCount,
				"min_required", 3,
				"hint", "save more memories to enable analysis",
			)
			continue
		}

		slog.Info("[Cognitive Engine] Analyzing namespace",
			"index", idx,
			"mode", g.cfg.Mode,
			"vectors", info.VectorCount,
			"llm_configured", g.llm != nil,
		)
	...
	g.reflectionsMu.Lock()
	reflectionCount := len(g.newReflections)
	g.reflectionsMu.Unlock()

	if reflectionCount > 0 {
		slog.Info("[Cognitive Engine] Sleep cycle completed. New reflections generated",
			"count", reflectionCount,
		)
	} else {
		slog.Info("[Cognitive Engine] Sleep cycle completed. No new reflections (detectors found no candidates)",
			"mode", g.cfg.Mode,
			"llm_configured", g.llm != nil,
		)
	}
...
func (g *Gardener) findRedundantClusters(indexName string, similarityThreshold float64, minClusterSize int) []MemoryCluster {
	...
	if err != nil || len(ids) == 0 {
		slog.Debug("[Cognitive Engine] Consolidation: no IDs to scan", "index", indexName, "err", err)
		return nil
	}
	...
	if err != nil {
		slog.Debug("[Cognitive Engine] Consolidation: VGetMany failed", "index", indexName, "err", err)
		return nil
	}
	...
	if scanned > 0 {
		slog.Debug("[Cognitive Engine] Consolidation scan",
			"index", indexName,
			"scanned", scanned,
...
func (g *Gardener) detectContradictions(indexName string) {
	...
	if err != nil || len(ids) == 0 {
		slog.Debug("[Cognitive Engine] Contradictions: no IDs to scan", "index", indexName, "err", err)
		return
	}
	g.setCursor(indexName+"_contradictions", nextCursor)

	nodes, _ := g.eng.VGetMany(indexName, ids)
	slog.Debug("[Cognitive Engine] Contradictions: scanning batch",
		"index", indexName,
...
type UserProfile struct {
	...
	ProfileData        string   `json:"profile_data"` // Last known LLM-extracted profile (sanitized, no recursion)
}

// UserProfileSnapshot is the serialization-safe view of UserProfile, used
// to populate ProfileData. It omits the ProfileData field itself to break
// the recursive nesting that previously grew to 18 levels / 16 MB.
...
// snapshot returns a JSON serialization of the profile with ProfileData
// excluded, breaking the self-referencing recursion.
func (p UserProfile) snapshot() string {
...
func (g *Gardener) UpdateUserProfile(indexName, userID string) error {
	...
	// Per-user lock: serialize concurrent UpdateUserProfile calls for the same
	// userID. Without this, when the LLM call is slow (10-20s for reasoning
	// models), a second debounce can fire while the first is still running,
	// causing both to read the same stale currentProfile and the second to
	// overwrite the first with a lower interaction_count.
	muIface, _ := g.profileUpdateLocks.LoadOrStore(userID, &sync.Mutex{})
	...
	// 1. Get recent memories for this user.
	// In MCP mode, any memory with user_id is implicitly a user interaction,
	// so we filter by type='memory' (not by tag) to get all memories attributed
	// to this user. The old `tags = 'user_interaction'` filter silently dropped
	// MCP memories that had no such tag.
	filter := fmt.Sprintf("user_id='%s' AND type='memory'", userID)
	ids, err := g.eng.VFilter(indexName, filter, g.cfg.ProfileUpdateThreshold*2)
	if err != nil {
	...
	// 4. Snapshot unassimilated count and reset to 0 BEFORE the LLM call.
	// This way, any new saves during the LLM call start a fresh counter
	// and will trigger another update, instead of being silently merged
	// into the current update.
	g.profileMutex.Lock()
	newInteractions := g.unassimilatedInteractions[userID]
	g.unassimilatedInteractions[userID] = 0
	g.profileMutex.Unlock()

	// 5. Generate updated profile (LLM or fallback)
	var updatedProfile UserProfile
	if g.llm != nil {
		updatedProfile = g.generateLLMProfileUpdate(currentProfile, interactions, timestamps, newInteractions)
	} else {
		updatedProfile = g.generateDeterministicProfileUpdate(currentProfile, interactions, newInteractions)
	}
	...
	profileJSON := updatedProfile.snapshot()
	meta["profile_data"] = profileJSON
	...
	// (unassimilated counter was already snapshotted and reset at step 4)
...
// newCount is the number of new unassimilated interactions since the last
// update (used for deterministic InteractionCount and Confidence fields).
func (g *Gardener) generateLLMProfileUpdate(current UserProfile, interactions, timestamps []string, newCount int) UserProfile {
...
4. Respond with ONLY a JSON object. No prose, no explanation, no thinking process. Do not wrap in markdown fences.
5. If you cannot extract a field, OMIT it from the JSON rather than setting it to null or empty string.
   - Omitted field = preserve current value
   - null = preserve current value
   - [] (empty array) = CLEAR the field to empty
   - ["value"] = replace with new value

RESPONSE LENGTH MAPPING (use these categories):
- "short" — 3-4 lines, brief, concise, sintentico
- "medium" — medium length, default
- "long" — verbose, detailed, lungo, dettagliato
...
Respond with JSON:
{
  ...
  "confidence": 0.0-1.0
	...
	userPrompt := "Based on the recent interactions above, produce the updated profile JSON."

	respText, err := g.llm.Chat(sysPrompt, userPrompt)
	if err != nil {
		slog.Warn("[Cognitive Engine] LLM profile update failed, using fallback", "error", err)
		return g.generateDeterministicProfileUpdate(current, interactions, newCount)
	}

	cleaned := cleanLLMJSONResponse(respText)
	if cleaned == "" {
		slog.Warn("[Cognitive Engine] No JSON object found in LLM profile response, retrying with stricter prompt",
			"raw_response_len", len(respText),
		)
		// Retry once with a stricter user prompt. Reasoning models sometimes
		// exhaust their output token budget on thinking, leaving no room for
		// the actual JSON. A second explicit nudge often succeeds.
		retryPrompt := "Your previous response did not contain valid JSON. " +
			"Output ONLY a JSON object now. No prose, no explanation, no thinking process, no markdown fences. " +
			"Just the JSON object matching the schema in the system prompt."
		respText2, err2 := g.llm.Chat(sysPrompt, retryPrompt)
		if err2 == nil {
			cleaned = cleanLLMJSONResponse(respText2)
		}
		if cleaned == "" {
			slog.Warn("[Cognitive Engine] Retry also failed, using deterministic fallback",
				"retry_len", len(respText2),
			)
			return g.generateDeterministicProfileUpdate(current, interactions, newCount)
		}
	}
	...
	if err := json.Unmarshal([]byte(cleaned), &resp); err != nil {
		slog.Warn("[Cognitive Engine] Failed to parse LLM profile response",
			"error", err,
			"cleaned_preview", previewForLog(cleaned, 200),
		)
		return g.generateDeterministicProfileUpdate(current, interactions, newCount)
	}

	// Merge strategy: null/missing = preserve current, [] = clear.
	// Determine which fields were present in the LLM response (vs omitted/null).
	merged := mergeProfileResponse(current, cleaned, resp)

	// Override deterministic fields (never trust the LLM with these)
	merged.InteractionCount = current.InteractionCount + newCount
	merged.LastUpdated = time.Now().Unix()
	if merged.Confidence == 0 {
		merged.Confidence = math.Min(1.0, float64(merged.InteractionCount)/50.0)
	}
	return merged
}

// mergeProfileResponse applies the merge strategy:
// - field omitted or null in LLM response → keep current value
// - field is [] (empty array) → clear to empty
// - field has values → replace
func mergeProfileResponse(current UserProfile, cleanedJSON string, llmResp struct {
	UserID             string   `json:"user_id"`
	CommunicationStyle string   `json:"communication_style"`
	Language           string   `json:"language"`
	ExpertiseAreas     []string `json:"expertise_areas"`
	...
	// String fields: only override if LLM provided a non-empty value.
	// The parser unmarshals both null and missing as "", so we can't
	// distinguish them. Treat empty as "preserve current" (safest default).
	if llmResp.CommunicationStyle != "" {
		merged.CommunicationStyle = llmResp.CommunicationStyle
	}
	if llmResp.Language != "" {
		merged.Language = llmResp.Language
	}
	if llmResp.ResponseLength != "" {
		merged.ResponseLength = llmResp.ResponseLength
	}

	// Array fields: distinguish null (preserve) vs [] (clear) vs values (replace)
	merged.ExpertiseAreas = resolveArrayField(cleanedJSON, "expertise_areas", current.ExpertiseAreas, llmResp.ExpertiseAreas)
	merged.Dislikes = resolveArrayField(cleanedJSON, "dislikes", current.Dislikes, llmResp.Dislikes)

	if llmResp.Confidence > 0 {
		merged.Confidence = llmResp.Confidence
	}

	return merged
}

// resolveArrayField implements the merge strategy for array fields:
// - key missing or null in JSON → return currentValues (preserve)
// - key is "[]" → return empty slice (clear)
// - key has values → return llmValues (replace)
func resolveArrayField(jsonStr, key string, currentValues, llmValues []string) []string {
	keyWithColon := fmt.Sprintf("\"%s\"", key)

	keyIdx := strings.Index(jsonStr, keyWithColon)
	if keyIdx == -1 {
		// Key not present in JSON → preserve current
		return currentValues
	}

	// Find what comes after the key: skip whitespace, colon, whitespace
	rest := jsonStr[keyIdx+len(keyWithColon):]
	...
	if strings.HasPrefix(rest, "null") {
		// Explicitly null → preserve current
		return currentValues
	}
	...
	// Has actual values → use LLM's parsed values
	return llmValues
}

// previewForLog returns a short preview of a string for log messages,
// safely handling short strings.
...
func (g *Gardener) generateDeterministicProfileUpdate(current UserProfile, interactions []string, newCount int) UserProfile {
	updated := current
	updated.InteractionCount = current.InteractionCount + newCount
	...
	if strings.Contains(lowerText, "breve") || strings.Contains(lowerText, "conciso") || strings.Contains(lowerText, "short") || strings.Contains(lowerText, "sintetico") {
		updated.ResponseLength = "short"
	} else if strings.Contains(lowerText, "lungo") || strings.Contains(lowerText, "verbose") || strings.Contains(lowerText, "long") || strings.Contains(lowerText, "dettagliato") {
		updated.ResponseLength = "long"
	} else if strings.Contains(lowerText, "medio") || strings.Contains(lowerText, "medium") {
		updated.ResponseLength = "medium"
	}
	...
	// --- Communication style ---
	if strings.Contains(lowerText, "tecnico") || strings.Contains(lowerText, "technical") {
		updated.CommunicationStyle = "technical"
	} else if strings.Contains(lowerText, "verbose") || strings.Contains(lowerText, "dettagliato") || strings.Contains(lowerText, "lungo") {
		updated.CommunicationStyle = "verbose"
	} else if strings.Contains(lowerText, "conciso") || strings.Contains(lowerText, "breve") || strings.Contains(lowerText, "sintetico") {
		updated.CommunicationStyle = "concise"
	}

	// --- Expertise areas (extract from common technical keywords) ---
	expertiseKeywords := map[string][]string{
		...
		"Databases":  {"database", "vector db", "hnsw"},
		"AI/ML":      {"machine learning", "neural", "embedding", "llm"},
	}
	expertiseSet := make(map[string]bool)
	for _, area := range updated.ExpertiseAreas {
		expertiseSet[area] = true
	}
	for area, keywords := range expertiseKeywords {
		for _, kw := range keywords {
			if strings.Contains(lowerText, kw) {
				if !expertiseSet[area] {
					updated.ExpertiseAreas = append(updated.ExpertiseAreas, area)
					expertiseSet[area] = true
				}
				break
			}
	...
	// --- Dislikes (deduplicated) ---
	dislikeSet := make(map[string]bool)
	for _, d := range updated.Dislikes {
		dislikeSet[d] = true
	}
	dislikePatterns := []struct {
		...
		{"senza markdown", "no markdown"},
		{"no emoji", "no emoji"},
		{"no java", "no Java"},
		{"avoid javascript", "avoid JavaScript"},
	}
	for _, p := range dislikePatterns {
		if strings.Contains(lowerText, p.keyword) && !dislikeSet[p.label] {
			updated.Dislikes = append(updated.Dislikes, p.label)
			dislikeSet[p.label] = true
		}
	}

	// --- Snapshot (no recursion) ---
	updated.ProfileData = updated.snapshot()
