...
# KektorDB Technical Documentation (v0.6.1)
...
*   `!=`: Not equal - includes nodes where the field does not match the value OR where the field does not exist
*   `<`, `<=`, `>`, `>=`: Numeric comparisons (require B-Tree index)
*   `AND`, `OR`: Logical operators (case insensitive)

> **Note on `!=` operator:** The not-equal operator includes nodes that don't have the specified field at all. For example, `_archived != 'true'` returns nodes with `_archived=false` as well as nodes without the `_archived` field. This is useful for "exclude archived" filters but may require explicit field existence checks if you need to ensure the field is present.
...
### 5.10 Memory Evolution (Semantic Git)
...
- TypeScript: `vbeliefState()`, `invalidateMemory()`, `vevolve()`, `getMemoryEvolution()`
